...
function msshext_elementor_load_plugin() {
	...
	if ( ! did_action( 'elementor_pro/loaded' ) ) {
		add_action( 'admin_notices', 'msshext_elementor_pro_fail_load' );
...
function msshext_elementor_fail_load() {
	...
	if ( _is_elementor_installed() ) {
		...
		$message = '<p>' . __( 'Elementor Powerup is not working because you need to activate the Elementor plugin.', 'msshext' ) . '</p>';
		$message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $activation_url, __( 'Activate Elementor Now', 'msshext' ) ) . '</p>';
	} else {
		...
		$message = '<p>' . __( 'Elementor Powerup is not working because you need to install the Elementor plugin.', 'msshext' ) . '</p>';
		$message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $install_url, __( 'Install Elementor Now', 'msshext' ) ) . '</p>';
	}
...
function msshext_elementor_fail_load_out_of_date() {
	...
	$message = '<p>' . __( 'Elementor Pro is not working because you are using an old version of Elementor.', 'msshext' ) . '</p>';
	$message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $upgrade_link, __( 'Update Elementor Now', 'msshext' ) ) . '</p>';
...
function msshext_elementor_admin_notice_upgrade_recommendation() {
	...
	$upgrade_link = wp_nonce_url( self_admin_url( 'update.php?action=upgrade-plugin&plugin=' ) . $file_path, 'upgrade-plugin_' . $file_path );
	$message = '<p>' . __( 'A new version of Elementor is available. For better performance and compatibility of Elementor Pro, we recommend updating to the latest version.', 'msshext' ) . '</p>';
	$message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $upgrade_link, __( 'Update Elementor Now', 'msshext' ) ) . '</p>';

	echo '<div class="error">' . $message . '</div>';
}

function msshext_elementor_pro_fail_load() {
	$screen = get_current_screen();
	if ( isset( $screen->parent_file ) && 'plugins.php' === $screen->parent_file && 'update' === $screen->id ) {
		return;
	}
	...
	if ( _is_elementor_pro_installed() ) {
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return;
		}
		...
		$message = '<p>' . __( 'MŠ Škola hrou extensions is not working because you need to activate the Elementor Pro plugin.', 'msshext' ) . '</p>';
		$message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $activation_url, __( 'Activate Elementor Pro Now', 'msshext' ) ) . '</p>';
	} else {
		$message = '<p>' . __( 'MŠ Škola hrou extensions is not working because you need to install and activate the Elementor Pro plugin.', 'msshext' ) . '</p>';
	}

	echo '<div class="error"><p>' . $message . '</p></div>';
}

if ( ! function_exists( '_is_elementor_pro_installed' ) ) {
	...
	function _is_elementor_pro_installed() {
		...
		return is_plugin_active( $file_path ) || file_exists( WP_PLUGIN_DIR . '/' . $file_path );
	}
}
